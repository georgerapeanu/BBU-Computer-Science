    ...
    )
{
    ...
    if (*State == CONN_UNAUTHENTICATED)
    {
        ...
        else if (0 == _stricmp(Command, CMD_RAND))
        {
            // Command = 'rand', Parameter = how many numbers to generate
            if (NULL == Parameter)
            {
                SetReply(Output, OutLength, "[ERROR] Do not know how many numbers to generate.");
                return TRUE;
            }

            if (!CmdHandleRand(Parameter, Output, OutLength))
            {
                SetReply(Output, OutLength, "[ERROR] Cannot generate random numbers.");
            }

            return TRUE;
        }
        else if (0 == _stricmp(Command, CMD_RAND_QUOTE))
        {
            // Command = 'randq', Parameter = quote index
            if (NULL == Parameter)
            {
                SetReply(Output, OutLength, "[ERROR] Do not know how which quote to return.");
                return TRUE;
            }

            if (!CmdHandleRandQ(Parameter, Output, OutLength))
            {
                SetReply(Output, OutLength, "[ERROR] Cannot return quote.");
            }
