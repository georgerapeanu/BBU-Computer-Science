...
impl Matrix {
  pub fn new(n: usize, m: usize) -> Self {
    Self {
      n,
      m,
      elems: (0..n*m).map(|_| rand::random::<i16>()).collect()
    }
  }
}
...
pub fn multiply_range(a: Arc<Matrix>, b: Arc<Matrix>, start: (usize, usize), gap: usize, count: usize, tx: mpsc::Sender<i64>) {
  let output_m = b.m;
  let mut next_cell = start;

  for _ in 0..count {
    tx.send(single_cell_multiply(a.clone(), b.clone(), &next_cell) as i64).expect("Send error");
    next_cell.1 += gap;
...
fn main() {
  ...
  let multiply_rows: Box<dyn Fn(_) -> i64> = Box::new(move |executor: Arc<dyn Fn(Box<dyn Fn() + Send>)>| -> i64 {
    ...
    (0..a_clone.n).for_each(|row| {
      ...
      executor(
        Box::new(
          move  || multiply_range(a_clone.clone(), b_clone.clone(), (row, 0), 1, b_clone.m, tx.clone())
        )
      );
    });
    
    rx.iter().take(a_clone.n * b_clone.m).sum()
  });
  ...
  let multiply_columns: Box<dyn Fn(_) -> i64> = Box::new(move |executor: Arc<dyn Fn(Box<dyn Fn() + Send>)>| -> i64 {
    ...
    (0..b_clone.m).for_each(|column| {
      ...
      executor(
        Box::new(
          move || multiply_range(a_clone.clone(), b_clone.clone(), (0, column), b_clone.m, a_clone.n, tx.clone())
        )
      );
    });
  ...
  let multiply_gap: Box<dyn Fn(_) -> i64> = Box::new(move |executor: Arc<dyn Fn(Box<dyn Fn() + Send>)>| -> i64 {
    ...
    (0..workers).for_each(|id| {
      ...
      executor(
        Box::new(
          move || multiply_range(a_clone.clone(), b_clone.clone(), start_cell, workers, reps, tx.clone())
        )
      );
    });
