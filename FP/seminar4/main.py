...
def delete_student_by_id(students,id):
  """this function removes the student with the given id from the student list
     The function changes the list given as parameter
  ...
  if find_by_id(students,id) == None:
    raise Exception("No such students exist")
  students[:] = filter(lambda x: get_id(x) != id,students)

def find_students_containing_string(students,s):
  """this function returns a list containing all students that contain s in their name
     case insensitive 

     students: list of students
     s: string

     returns list of students matching the criteria
  """

  ans = list(filter(lambda x: get_name(x).lower().find(s.lower()) != -1,students))
  if len(ans) == 0:
...
def ui_find_students_containing_string(students, s):
  ans = find_students_containing_string(students,s)
  print(ans)

def ui_delete_student_by_id(students,id):
  delete_student_by_id(students,int(id))
...
def run_menu_cmd():
    ...
    commands = {
        ...
        "delete": ui_delete_student_by_id,
        "find": ui_find_students_containing_string
    }
...
def test_delete_student_by_id():
  students = set_up()
  delete_student_by_id(students,1)
  assert students == [create_student(2,"Radu",2)]
  assert had_exception(delete_student_by_id,(students,1)) 

def test_find_students_containing_string():
  students = set_up()
  assert find_students_containing_string(students,"a") == [create_student(1,"Alex",1),create_student(2,"Radu",2)]
  assert find_students_containing_string(students,"Ra") == [create_student(2,"Radu",2)]
  assert find_students_containing_string(students,"") == [create_student(1,"Alex",1),create_student(2,"Radu",2)]
  assert had_exception(find_students_containing_string,(students,"asdas"))

def test_all():
    test_add_student()
    test_delete_student_by_id()
    test_find_students_containing_string()
