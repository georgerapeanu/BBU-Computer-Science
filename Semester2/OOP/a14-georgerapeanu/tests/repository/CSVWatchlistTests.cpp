/*!
 * @file CSVWatchlistTests.cpp this file contains the tests for the repository
 */
...
class CSVWatchlistTests:public testing::Test{
protected:
    CSVWatchlist<Movie> repository;

    CSVWatchlistTests():repository(CSVWatchlist<Movie>("./test.csv")){
        ;
    }

    void SetUp()override{
        remove("./test.csv");
        repository = CSVWatchlist<Movie>("./test.csv");
...
TEST_F(CSVWatchlistTests, test_add){
...
TEST_F(CSVWatchlistTests, test_update){
    EXPECT_THROW(repository.add(Movie("movie1", "genre3", 1, 10, "trailer3")), RepositoryException);
...
TEST_F(CSVWatchlistTests, test_remove){
    EXPECT_THROW(repository.add(Movie("movie1", "genre3", 1, 10, "trailer3")), RepositoryException);
...
TEST_F(CSVWatchlistTests, test_find){
