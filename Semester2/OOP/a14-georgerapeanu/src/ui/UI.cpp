...
UI::UI(MovieController &controller): controller(controller) {
    ;
}

void UI::run_admin() {
    vector<std::string> help_display;
    ...
    while(true){
        ...
        vector<std::string> args = split_command(command);
        try{
...
void UI::run_user() {
    ...
    help_display.push_back("logout -> logout of the admin account");
    help_display.push_back("help -> displays the help prompt");
    help_display.push_back("display watchlist -> displays the watch list");
    help_display.push_back("===========================================");
    display_help(help_display);
    while(true){
        std::string command;
        ...
        try{
            if(args.size() > 3 && args[0] == "get" && args[1] == "movies" && args[2] == "from"){
                ...
                break;
            }else if(args.size() == 2 && args[0] == "display" && args[1] == "watchlist"){
...
vector<std::string> UI::split_command(const std::string &s) {
...
void UI::handle_add_admin(const vector<std::string> &args) {
    if(args.size() != 6){
...
void UI::handle_update_admin(const vector<std::string> &args) {
    if(args.size() != 6){
        throw UIException("arguments are wrong for given command");
    }
...
void UI::handle_remove_admin(const vector<std::string> &args) {
    if(args.size() != 3){
        throw UIException("arguments are wrong for given command");
    }
    try {
...
void UI::handle_get_movies_admin(const vector<std::string> &args) {
    if(args.size() != 1){
        throw UIException("arguments are wrong for given command");
    }
    vector<Movie> movies = this->controller.get_all_movies_admin();
    for(unsigned int i = 0;i < movies.size();i++){
        display_movie(movies[i]);
    }
}

void UI::handle_get_movies_user(const vector<std::string> &args) {
    if(args.size() != 4){
        throw UIException("arguments are wrong for given command");
    }
    vector<Movie> movies = this->controller.get_movies_user(args[3]);
    if(movies.size() == 0){
...
void UI::handle_remove_movie_user(const vector<std::string> &args) {
    if(args.size() != 4){
        throw UIException("arguments are wrong for given command");
    }
    Movie movie;
...
void UI::handle_get_watchlist_user(const vector<std::string> &args) {
    if(args.size() != 1){
        throw UIException("arguments are wrong for given command");
    }
    vector<Movie> movies = this->controller.get_watchlist_user();
    for(unsigned int i = 0;i < movies.size();i++){
        display_movie(movies[i]);
    }
}

void UI::display_help(const vector<std::string> &help) const {
