...
Movie::Movie(const Movie &other){
    ...
    MovieValidator::validate_movie(*this);
}
...
std::istream &operator>>(std::istream &in, Movie &c) {
    in >> c.title >> c.genre >> c.likes >> c.year >> c.trailer_link;
    MovieValidator::validate_movie(c);
    return in;
}

std::ostream &operator<<(std::ostream &out, Movie c) {
    out << c.title << " " << c.genre << " " << c.likes << " " << c.year << " " << c.trailer_link;
    return out;
}
