...
def main():
    ...
    help_prompt = "\n".join(
            ...
            "smallest walk [u] [v]",
            "\tgets the smallest cost walk from u to v",
            "help",
            "exit"
            ])
    ...
    while True:
        ...
        try:
            if re.match(commands_regex[0], command) is not None:
                ...
                answer = Graph.floyd_warshall(graph, args[2], args[3])
                if answer[0] == inf:
                    raise Exception(f"No walk between {args[2]} and {args[3]}")
                print(f"Walk with cost {answer[0]} found!")
