...
def get_minimum_spanning_tree(graph):
    ...
    :param graph: the graph
    :type graph: UndirectedGraph
    :return: the tree as an UndirectedGraph
    """
    def find_root(father, node):
        """ Find root function for DSU

        :param father: a dictionary containing the father of each node
        :type father: dict
        :param node: the node for which the root to be found in the DSU
        :type node: str
        :return: str
        """
        if father[node] == "":
            return node
        father[node] = find_root(father, father[node])
        return father[node]

    def unite(father, x, y):
        """ Union root function for DSU
        ...
        """
        x = find_root(father, x)
        y = find_root(father, y)
        if x == y:
            return False
        father[x] = y
        return y
    father = {}  # For DSU structure

    edges = []

    for node in graph.parse_vertices():
        father[node] = ""
        for neighbor in graph.parse_adjacent_edges(node):
            edges.append((node, neighbor, graph.get_edge_cost(node, neighbor)))

    answer = UndirectedGraph(list(graph.parse_vertices()),[])

    edges = sorted(edges, key=(lambda edge:edge[2]))

    for edge in edges:
        if unite(father, edge[0], edge[1]):
            answer.add_edge(edge[0], edge[1], edge[2])
