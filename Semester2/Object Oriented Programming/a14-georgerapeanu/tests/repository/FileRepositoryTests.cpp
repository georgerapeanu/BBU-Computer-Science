/*!
 * @file FileRepositoryTests.cpp this file contains the tests for the repository
 */
...
class FileRepositoryTests:public testing::Test{
protected:
    FileRepository<Movie> repository;

    FileRepositoryTests():repository(FileRepository<Movie>("./test.txt")){
        ;
    }

    void SetUp()override{
        remove("./test.txt");
        repository = FileRepository<Movie>("./test.txt");
...
TEST_F(FileRepositoryTests, test_add){
...
TEST_F(FileRepositoryTests, test_update){
    EXPECT_THROW(repository.add(Movie("movie1", "genre3", 1, 10, "trailer3")), RepositoryException);
...
TEST_F(FileRepositoryTests, test_remove){
    EXPECT_THROW(repository.add(Movie("movie1", "genre3", 1, 10, "trailer3")), RepositoryException);
...
TEST_F(FileRepositoryTests, test_find){
